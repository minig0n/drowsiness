...
# iris_model = IrisLocalizationModel("../weights/iris_landmark.tflite")
...
while True:
    ...
    for face in faces:
        ...
        eye_y_max = landmarks[0][1] + size/2
        eye_y_min = landmarks[0][1] - size/2
        eye_x_min = landmarks[0][0] - size/2
        eye_x_max = landmarks[0][0] + size/2
        center = (float(landmarks[0][0]), float(landmarks[0][1]))
        
        M_left = cv2.getRotationMatrix2D(center, angle, scale=1.0)
        rotated_left = cv2.warpAffine(frame, M_left, (frame.shape[0], frame.shape[1]))
        rotated_cropped = rotated_left[int(eye_y_min):int(eye_y_max), int(eye_x_min):int(eye_x_max)]
        cv2.imshow('left_eye', cv2.resize(rotated_cropped, (200, 200)))

        eye_y_max = landmarks[1][1] + size/2
        eye_y_min = landmarks[1][1] - size/2
        eye_x_min = landmarks[1][0] - size/2
        eye_x_max = landmarks[1][0] + size/2
        center = (float(landmarks[1][0]), float(landmarks[1][1]))

        M_left = cv2.getRotationMatrix2D(center, angle, scale=1.0)
        rotated_right = cv2.warpAffine(frame, M_left, (frame.shape[0], frame.shape[1]))
        rotated_cropped = rotated_right[int(eye_y_min):int(eye_y_max), int(eye_x_min):int(eye_x_max)]
        cv2.imshow('right_eye', cv2.resize(rotated_cropped, (200, 200)))
